...
} from '../utils/danishDicitonary.ts/mapDaEng';
  ...
  try {
    ...
    async function processDanishFixedExpressions(
      ...
      for (const expr of expressions) {
        ...
        // Create English translation word
        const englishExprWord = await tx.word.upsert({
          where: {
            word_languageCode: {
              word: expr.expression_translation_en,
              languageCode: LanguageCode.en,
            },
          },
          update: {},
        ...
        await tx.wordRelationship.upsert({
          where: {
            fromWordId_toWordId_type: {
              fromWordId: expressionWord.id,
              toWordId: englishExprWord.id,
              type: RelationshipType.translation,
            },
          },
          update: {},
    ...
    async function processDanishVariants(
      ...
      for (const variant of variants) {
        // Skip if it's the same as the main word
        if (variant.word.word === danish_word_data.word.word) continue;
